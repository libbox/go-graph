...
func (uc *UserCreate) AddRequesterIDs(ids ...int) *UserCreate {
	uc.mutation.AddRequesterIDs(ids...)
	return uc
}

// AddRequesters adds the requesters edges to User.
func (uc *UserCreate) AddRequesters(u ...*User) *UserCreate {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
	return uc.AddRequesterIDs(ids...)
}

// AddRequestingIDs adds the requesting edge to User by ids.
func (uc *UserCreate) AddRequestingIDs(ids ...int) *UserCreate {
	uc.mutation.AddRequestingIDs(ids...)
	return uc
}

// AddRequesting adds the requesting edges to User.
func (uc *UserCreate) AddRequesting(u ...*User) *UserCreate {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
	return uc.AddRequestingIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.RequestersIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
	...
	if nodes := uc.mutation.RequestingIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
