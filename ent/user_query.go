...
type UserQuery struct {
	...
	withRequesters *UserQuery
	withRequesting *UserQuery
...
// QueryRequesters chains the current query on the requesters edge.
func (uq *UserQuery) QueryRequesters() *UserQuery {
	query := &UserQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
// QueryRequesting chains the current query on the requesting edge.
func (uq *UserQuery) QueryRequesting() *UserQuery {
	query := &UserQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
//  WithRequesters tells the query-builder to eager-loads the nodes that are connected to
// the "requesters" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithRequesters(opts ...func(*UserQuery)) *UserQuery {
	query := &UserQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withRequesters = query
	return uq
}

//  WithRequesting tells the query-builder to eager-loads the nodes that are connected to
// the "requesting" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithRequesting(opts ...func(*UserQuery)) *UserQuery {
	query := &UserQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withRequesting = query
	return uq
}

//  WithFriend tells the query-builder to eager-loads the nodes that are connected to
// the "friend" edge. The optional arguments used to configure the query builder of the edge.
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	var (
		...
		loadedTypes = [6]bool{
			...
			uq.withRequesters != nil,
			uq.withRequesting != nil,
			uq.withFriend != nil,
			uq.withLikes != nil,
		}
	)
	...
	if query := uq.withRequesters; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*User, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.RequestersPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "requesters": %v`, err)
		}
		query.Where(user.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "requesters" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Requesters = append(nodes[i].Edges.Requesters, n)
			}
		}
	}

	if query := uq.withRequesting; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*User, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.RequestingPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "requesting": %v`, err)
		}
		query.Where(user.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "requesting" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Requesting = append(nodes[i].Edges.Requesting, n)
			}
		}
	}
	...
	if query := uq.withFriend; query != nil {
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "friend": %v`, err)
		}
		query.Where(user.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
