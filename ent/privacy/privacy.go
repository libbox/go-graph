...
// The ForumQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type ForumQueryRuleFunc func(context.Context, *ent.ForumQuery) error

// EvalQuery return f(ctx, q).
func (f ForumQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.ForumQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.ForumQuery", q)
}

// The ForumMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type ForumMutationRuleFunc func(context.Context, *ent.ForumMutation) error

// EvalMutation calls f(ctx, m).
func (f ForumMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.ForumMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.ForumMutation", m)
}
